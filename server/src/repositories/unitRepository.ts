...
const columns = `u.id, u.property_id AS "propertyId", p.name AS "propertyName", p.active AS "propertyActive",
  ...
  u.occupancy_status AS status, u.active_lease_id AS "activeLeaseId", u.has_upcoming_lease AS "hasUpcomingLease",
  u.notes, u.created_at AS "createdAt", u.updated_at AS "updatedAt"`;
...
export async function findUnit(database: Pool | PoolClient, companyId: string, id: string) {
  const result = await database.query(
    `SELECT ${columns} FROM unit_occupancy u ${join} WHERE u.company_id=$1 AND u.id=$2`,
    [companyId, id],
  );
...
export async function listUnits(database: Pool, companyId: string, query: UnitQuery) {
  ...
  const where = `u.company_id=$1 AND (u.name ILIKE $2 OR p.name ILIKE $2)
    AND ($3::text IS NULL OR u.occupancy_status=$3) AND ($4::uuid IS NULL OR u.property_id=$4)
    AND ($5='all' OR p.active=($5='active'))`;
  const [items, total] = await Promise.all([
    database.query(
      `SELECT ${columns} FROM unit_occupancy u ${join} WHERE ${where} ORDER BY p.name, u.name, u.id
      LIMIT $6 OFFSET $7`,
      [...params, query.pageSize, (query.page - 1) * query.pageSize],
    ),
    database.query(
      `SELECT count(*)::int AS total FROM unit_occupancy u ${join} WHERE ${where}`,
      params,
    ),
  ]);
  return {
